...
class ReplayBuffer:
    def __init__(self, capacity: int = 100_000, alpha: float = 0.6):
        ...
        alpha:
          ...
          1.0 -> fully proportional to priority
        """
        self.buffer = deque(maxlen=capacity)
        self.priorities = deque(maxlen=capacity)
        self.alpha = alpha

    def push(self, *args, priority: float = 1.0) -> None:
        self.buffer.append(Transition(*args))
        self.priorities.append(float(max(1e-6, priority)))

    def sample(self, batch_size: int):
        if batch_size > len(self.buffer):
            raise ValueError(f"sample batch_size={batch_size} > buffer size={len(self.buffer)}")

        scaled = np.asarray(self.priorities, dtype=np.float64) ** self.alpha
...
class DQNConfig:
    ...
    good_buffer_capacity: int = 20_000
    good_fraction: float = 0.25
    good_episode_rescue_threshold: int = 2
    ...
    # whole-episode / later-step bonuses
...
class DQNAgent:
    def __init__(self, env: SearchRescueEnv, device: str = "cpu", config: DQNConfig = DQNConfig()):
        ...
        self.buffer = ReplayBuffer(config.buffer_capacity, alpha=config.replay_priority_alpha)
        self.good_buffer = ReplayBuffer(config.good_buffer_capacity, alpha=config.replay_priority_alpha)
    ...
    def compute_transition_priority(
        ...
        # episode-level quality bonus
        p += cfg.prio_episode_score_scale * episode_score

        # later transitions in a strong path get slightly more weight
    ...
    def train_step(self) -> float | None:
        ...
        use_good = len(self.good_buffer) >= max(8, int(cfg.batch_size * cfg.good_fraction))
        good_bs = int(cfg.batch_size * cfg.good_fraction) if use_good else 0
        main_bs = cfg.batch_size - good_bs

        main_batch = self.buffer.sample(main_bs)

        if good_bs > 0:
            good_batch = self.good_buffer.sample(good_bs)
            batch = Transition(
                state_vec=main_batch.state_vec + good_batch.state_vec,
                action=main_batch.action + good_batch.action,
                reward=main_batch.reward + good_batch.reward,
                next_state_vec=main_batch.next_state_vec + good_batch.next_state_vec,
                done=main_batch.done + good_batch.done,
                legal_mask=main_batch.legal_mask + good_batch.legal_mask,
                next_legal_mask=main_batch.next_legal_mask + good_batch.next_legal_mask,
            )
        else:
            batch = main_batch
    ...
    def train(self, num_episodes: int = 1000, verbose_every: int = 20):
        ...
        for ep in range(1, num_episodes + 1):
            ...
            # After episode ends, compute whole-episode quality and then assign per-transition priority
            ...
            for idx, step in enumerate(episode_steps):
                ...
                self.buffer.push(*tr, priority=prio)

            is_good_episode = (
                ep_success
                or ep_rescued >= self.config.good_episode_rescue_threshold
                or ep_found >= self.env.total_victims
                or episode_score >= 6.0
            )
            ...
            if is_good_episode:
                for idx, step in enumerate(episode_steps):
                    ...
                    self.good_buffer.push(*tr, priority=prio)
            ...
            if ep % verbose_every == 0:
                ...
                self._log(
                    ...
                    f"good_buf={len(self.good_buffer)} | "
...
def main():
    ...
    env = build_default_env(seed=42, rows=15, cols=20)
    ...
    agent = DQNAgent(env=env, device=device, config=DQNConfig())

    path = "dqn_sar_best15_20.pt"
    agent.load(path)
    agent.train(num_episodes=600, verbose_every=5)
    agent.save(path)
